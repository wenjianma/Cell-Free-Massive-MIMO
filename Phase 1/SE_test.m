    ...
    for k = 1:K
        ...
        for l = 1:L
            ...
            for no = 1:numOfRealization
                hdw = hdw + power_alloc(k,l)* H(:, no, k, l)' * D(:, :, k, l) * w_MR(:, no,k, l)/numOfRealization;
            end
        ...
        for i = 1:K
            ...
            for l = 1:L
                hdw2 = 0;
                for no = 1: numOfRealization
                hdw2 = hdw2 + power_alloc(i,l)*H(:, no, k, l)' * D(:, :, i, l) * w_MR(:,no, i, l)/numOfRealization; 
                end
                hdw3 = hdw3 + hdw2;
            end
            hdw4 = abs(mean(abs(hdw3).^2,"all"));
            interference = interference + hdw4;
        end
